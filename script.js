...
if (contactForm) {
  contactForm.addEventListener('submit', async function(e) {
    ...
    // Opción 1: Usar FormSubmit.co (GRATIS - RECOMENDADO)
    try {
      ...
      formData.append('_subject', `Nuevo contacto SCESA - ${nombre}`);
      formData.append('_captcha', 'false');
      
      // CAMBIA ESTE EMAIL por el tuyo
      const response = await fetch('https://formsubmit.co/ajax/elpocam@gmsil.com', {
        method: 'POST',
        body: formData
      });
      ...
      alert('❌ Hubo un error al enviar el mensaje. Por favor, intente de nuevo o llámenos directamente.');
    } finally {
      submitBtn.innerHTML = originalText;
      submitBtn.disabled = false;
    }
    
    /* Opción 2: Usar EmailJS (requiere registro)
    // Registro gratis en emailjs.com
    try {
    ...
    */
    ...
    /* Opción 3: Solo alerta (no envía realmente)
    alert('¡Gracias! Te contactaremos pronto.');
    contactForm.reset();
    */
  });
